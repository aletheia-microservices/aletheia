...
func isBlueprintRelationalDBCall(graph *ssagraph.SSAGraph, call *ssa.Call, unOp *ssa.UnOp) (string, string, common.DatabaseOperationType, []ValFieldPath, bool) {
	...
	if typeNamed, ok := unOp.Type().(*types.Named); ok {
		if typeNamed.String() == BLUEPRINT_BACKEND_PACKAGE+".RelationalDB" {
			...
			case common.OP_READ:
				var tables []string
				writeOrReadFields, filterFields, tables, ok = appparser.ParseSQLRead(database, stmt)
				if !ok {
					return "", "", -1, nil, false
				}
				tableName = tables[0]
			...
			case common.OP_WRITE:
				var ok bool
				writeOrReadFields, _, tableName, ok = appparser.ParseSQLWrite(database, stmt)
				if !ok {
					return "", "", -1, nil, false
				}
			case common.OP_DELETE:
				var tables []string
				var ok bool
				filterFields, tables, ok = appparser.ParseSQLDelete(database, stmt)
				if !ok {
					return "", "", -1, nil, false
				}
				tableName = tables[0]
