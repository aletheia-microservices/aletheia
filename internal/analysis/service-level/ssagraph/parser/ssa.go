...
// RunSSAAnalysis builds the SSA graphs of pkg into funcGraphs and saves its SSA code to
// output/{app}/ssa/{pkg}.ssa
func RunSSAAnalysis(app *app.App, prog *ssa.Program, pkg *ssa.Package, funcGraphs map[string]*ssagraph.SSAGraph) {
	...
	RunSSAAnalysisTo(ssaFile, app, prog, pkg, funcGraphs)
}

// RunSSAAnalysisTo is the same as RunSSAAnalysis but writes the SSA code to ssaFile
// (e.g., io.Discard to skip it)
func RunSSAAnalysisTo(ssaFile io.Writer, app *app.App, prog *ssa.Program, pkg *ssa.Package, funcGraphs map[string]*ssagraph.SSAGraph) {
...
func iterateFunc(app *app.App, ssaFile io.Writer, fn *ssa.Function, funcGraphs map[string]*ssagraph.SSAGraph, goroutine bool) {
...
func parseInstr(app *app.App, graph *ssagraph.SSAGraph, instr ssa.Instruction, instrIdx int, visited map[ssa.Value]bool, ssaFile io.Writer, funcGraphs map[string]*ssagraph.SSAGraph) *ssagraph.SSANode {
