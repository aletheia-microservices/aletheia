	...
	for _, edge := range it.graph.GetEdgesFromNode(node) {
		if edge.GetEdgeType() == abstractgraph.EDGE_SERVICE_RPC {
			...
			for i, toParam := range toNode.GetParams() {
				...
				taintMappingTmp := abstractgraphtainter.MergeTaints(toParam, fromArg.GetAllTaintsBeforeT(edge.GetT()), nil, abstractgraphtainter.MERGE_MODE_TAINT, config.MIN_T, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
				taintMapping.Join(taintMappingTmp, true)
				taintMappingTmp = abstractgraphtainter.MergeTaints(toParam, fromArg.GetAllTaintsAfterT(edge.GetT()), nil, abstractgraphtainter.MERGE_MODE_TAINT, config.MAX_T, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
				taintMapping.Join(taintMappingTmp, true)
			}

			// update taints on future node
			abstractgraphtainter.PropagateTaintsToServiceCallObjects(it.graph, toNode, taintMapping, nil, true, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
			abstractgraphtainter.PropagateNewTaintsToDatabaseCallObjects(it.graph, toNode, taintMapping, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
			...
			if it.mode == PHASE_1_SCHEMA_BUILDER || it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY {
				abstractgraphtainter.PropagateNewTaintsToDatabaseSchemas(it.graph, it.currentReqIdx(), Config.IgnoreForeignKeys, taintMapping, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
			}
			...
			for i, fromArg := range edge.GetArguments() {
				...
				// logrus.Tracef("[TRANSVERSE] [ARG << PARAM] fromArg=%s // toParam=%s\n", fromArg.String(), toParam.String())
				taintMappingTmp := abstractgraphtainter.MergeTaints(fromArg, toParam.GetAllTaints(), nil, abstractgraphtainter.MERGE_MODE_TAINT, edge.GetT(), it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
				taintMapping.Join(taintMappingTmp, true)
			}
			...
			for i, fromRet := range edge.GetReturns() {
				...
				taintMappingTmp := abstractgraphtainter.MergeTaints(fromRet, toRet.GetAllTaints(), nil, abstractgraphtainter.MERGE_MODE_TAINT, edge.GetT(), it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
				taintMapping.Join(taintMappingTmp, true)
			}

			// update taints on current node
			abstractgraphtainter.PropagateTaintsToServiceCallObjects(it.graph, node, taintMapping, edge, false, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
			abstractgraphtainter.PropagateNewTaintsToDatabaseCallObjects(it.graph, node, taintMapping, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
...
func (it *Iterator) transverseQueue(node *abstractgraph.AbstractNode, currDB *backends.Database, edge *abstractgraph.AbstractEdge) {
	...
	if config.Global.PropagateTaintsAcrossQueueOperations {
		...
		for i, arg := range edge.GetArguments() {
			...
			taintMappingTmp := abstractgraphtainter.MergeTaints(otherArg, arg.GetPrimaryTaints(), nil, abstractgraphtainter.MERGE_MODE_TAINT, config.MIN_T, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
			taintMapping.Join(taintMappingTmp, true)
		}
		abstractgraphtainter.PropagateTaintsToServiceCallObjects(it.graph, node, taintMapping, queueReadEdge, true, it.mode == PHASE_1_SCHEMA_BUILDER_READ_ONLY)
