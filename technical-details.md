...
The `(read_val, read_key)` rule is disabled by default. To enable it, set `CreateReferencesFromReadReadPairAndValKey` in [`pkg/config/config.go`](./pkg/config/config.go).
