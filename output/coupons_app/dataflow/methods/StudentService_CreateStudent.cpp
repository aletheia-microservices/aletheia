...
[_1] (Reference UserType) ref <ctx context.Context> @ Frontend
    ...
    --> w-tainted: write(students_db.Student.StudentID) {1}
...
[0] (StructObject UserType) coupon coupons_app.Student struct{StudentID int, Name string, Balance int}
     --> w-tainted: write(students_db.Student.Name) {1}
...
[___3] (Reference BasicType) ref <name string> @ Frontend
     --> w-tainted: write(students_db.Student.StudentID) {1}
[_1] (FieldObject FieldType) StudentID int
      --> w-tainted: write(students_db.Student.StudentID) {1}
[__2] (BasicObject BasicType) studentID int
       --> w-tainted: write(students_db.Student.StudentID) {1}
